# Long Asset History Timing Baseline

Status: implemented; conclusions remain run-specific.
...
For resource and plan analysis, use the companion [diagnostics](../diagnostics/README.md).
...
Requires .NET 10 SDK for the report. From this directory:

```bash
dotnet build report.cs
bash -n run.sh
# Report smoke check against existing saved results; does not rerun the workload.
dotnet run report.cs
...
Keep existing SQL/C# formatting; no automatic formatter is configured. The full
`./run.sh` is a heavy benchmark and rebuilds the dataset; do not use it as a smoke test.
Reports overwrite the same run's files. No cleanup is needed; saved reports are ignored.
